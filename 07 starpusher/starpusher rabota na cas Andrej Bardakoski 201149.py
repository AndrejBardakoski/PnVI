...
import time  # need for calculating time_taken
...
# Change for requirement 3:  increasing the chance an outside tile to be decorated
...
def runLevel(levels, levelNum):
    ...
    mapObj = addRandomRock(mapObj, levelObj['startState']['player'], levelObj['startState']['stars'], levelObj['goals'])
    ...
    while True:  # main game loop
        ...
        # Change for requirement 2:
        if not levelIsComplete:  # if level is not finished show the time passed since the start
            time_taken = time.time() - start_time
        # draw the time taken message
        timeSurf = BASICFONT.render('Time taken: %ss' % (round(time_taken, 1)), 1, TEXTCOLOR)
    ...
    for x in range(len(map)):
        for y in range(len(map[0])):
            if map[x][y] == 'o':
                tiles.append((x, y))
    emptyTiles = []
    # make sure the rock won't be placed on the same tile as a star,a goal or the player
    for cords in tiles:
        if not ((cords in stars) or (cords in goals) or (cords in [player])):
            emptyTiles.append(cords)
    rockCordx, rockCordy = random.choice(emptyTiles)  # get a random tile where to put the rock from emptyTiles
